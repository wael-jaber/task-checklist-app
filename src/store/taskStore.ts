...
export const useTaskStore = create<TaskState, [['zustand/devtools', never]]>(
  devtools(
    (set, get) => ({
      tasks: [],
      selectedTaskId: null,
      ...
      updateTask: async (id: string, updates: Partial<Task>) => {
        ...
        try {
          ...
          const errorMessage =
            error instanceof Error ? error.message : 'Unknown error updating task';
          set({ error: errorMessage, isLoading: false });
        }
      },
      ...
      deleteTask: async (id: string) => {
        ...
        try {
          ...
          const errorMessage =
            error instanceof Error ? error.message : 'Unknown error deleting task';
          set({ error: errorMessage, isLoading: false });
        }
      },
    ...
    { name: 'TaskStore', enabled: process.env.NODE_ENV === 'development' } // Only enable in development mode
  )
);
